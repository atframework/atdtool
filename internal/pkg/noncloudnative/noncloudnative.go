// Package tcm provides functions for generating tcm deploy tool configuration files,
...
type Config struct {
	Center     *CenterConf     `module:"tcmcenter"`
...
type tcmConf interface {
	XMLExport(outPath string) error
}

var confLoader = map[string]func(string) (tcmConf, error){
	"tcmcenter":        loadCenterConfig,
...
// LoadConfig load tcm configuration data
	...
	for i := 0; i < rtyp.NumField(); i++ {
		...
		if ok {
			...
			for _, cfgPath := range cfgPaths {
				if walkErr := filepath.Walk(cfgPath, func(filename string, fi os.FileInfo, err error) error {
					...
					if !fi.IsDir() && strings.Compare(fi.Name(), name) == 0 {
						// the tcm module configuration will be completely replaced
			...
			if rs == nil || reflect.TypeOf(rs).Kind() != reflect.Ptr || reflect.ValueOf(rs).IsNil() {
				return nil, fmt.Errorf("load tcm configuration file(%s) not found", name)
			}
