...
// ZoneLayer is tcm zone layer configuration
...
// WorldLayer is tcm world layer configuration
...
// ClusterLayer is tcm cluster layer configuration
...
// tcmProcGroupNode represents a proc group node in xml configuration
type tcmProcGroupNode struct {
	Name  string           `xml:"Name,attr"`
	Layer string           `xml:"Layer,attr"`
	Procs []*ProcBriefNode `xml:"Proc"`
}

// tcmProcConf represents a proc description in xml configuration
type tcmProcConf struct {
	XMLName xml.Name `xml:"tcmcenter"`
	Cluster ClusterLayer
	Groups  []*tcmProcGroupNode `xml:"ProcGroup"`
}

func loadProcConfig(filename string) (tcmConf, error) {
	config := new(ProcConf)
...
// XMLExport export tcm proc configuration
func (c *ProcConf) XMLExport(outPath string) error {
	tcmCfg := &tcmProcConf{}
	tcmCfg.Cluster.LayerNodeAttr = c.ClusterAttr

	for _, deployGroup := range c.Groups {
		g := &tcmProcGroupNode{Name: deployGroup.Name, Layer: deployGroup.Layer}
		for _, FuncName := range deployGroup.Procs {
			...
			case LayerCluster:
				tcmCfg.Cluster.Procs = append(tcmCfg.Cluster.Procs, proc)
			case LayerWorld:
				if tcmCfg.Cluster.World == nil {
					tcmCfg.Cluster.World = &WorldLayer{}
					tcmCfg.Cluster.World.LayerNodeAttr = c.WorldAttr
				}
				tcmCfg.Cluster.World.Procs = append(tcmCfg.Cluster.World.Procs, proc)
			case LayerZone:
				if tcmCfg.Cluster.World == nil {
					tcmCfg.Cluster.World = &WorldLayer{}
					tcmCfg.Cluster.World.LayerNodeAttr = c.WorldAttr
				...
				if tcmCfg.Cluster.World.Zone == nil {
					tcmCfg.Cluster.World.Zone = &ZoneLayer{}
					tcmCfg.Cluster.World.Zone.LayerNodeAttr = c.ZoneAttr
				}

				tcmCfg.Cluster.World.Zone.Procs = append(tcmCfg.Cluster.World.Zone.Procs, proc)
			default:
				return fmt.Errorf("layer[%s]: invalid", deployGroup.Layer)
			}
		}

		tcmCfg.Groups = append(tcmCfg.Groups, g)
	}

	if tcmCfg.Cluster.World.Zone == nil {
		tcmCfg.Cluster.World.Zone = new(ZoneLayer)
		tcmCfg.Cluster.World.Zone.LayerNodeAttr = c.ZoneAttr
	}

	outfile := path.Join(outPath, "proc.xml")
	output, err := xml.MarshalIndent(tcmCfg, "", "    ")
	if err != nil {
