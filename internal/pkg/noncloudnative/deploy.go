...
// tcmDeployGroup represents a deploy group attribute
type tcmDeployGroup struct {
...
// tcmZoneDeploy represents a tcm zone deploy
type tcmZoneDeploy struct {
	XMLName xml.Name `xml:"zone"`
	DeployNodeAttr
	CustomAttr string `xml:"CustomAttr,attr,omitempty"`
	Groups     []*tcmDeployGroup
}

// tcmWorldDeploy represents a tcm world deploy
type tcmWorldDeploy struct {
...
// tcmClusterDeploy represents a tcm cluster deploy
type tcmClusterDeploy struct {
	XMLName xml.Name `xml:"ClusterDeploy"`
	DeployNodeAttr
	CustomAttr string `xml:"CustomAttr,attr,omitempty"`
	Groups     []*tcmDeployGroup
	Worlds     []*tcmWorldDeploy
}

// tcmProcDeployConf represents a tcm configuration
type tcmProcDeployConf struct {
	XMLName xml.Name `xml:"tcmcenter"`
	Cluster tcmClusterDeploy
}

func loadProcDeployData(filename string) (tcmConf, error) {
...
func (m *tcmDeployGroup) setValue(name string, node *DeployNode, idx int) {
	if node != nil {
		m.Group = name
		m.Host = node.Host
...
func createDeployNodes(procGroup string, nodes []*DeployNode) []*tcmDeployGroup {
	groups := make([]*tcmDeployGroup, 0)
...
// XMLExport export tcm proc deploy xml configuration
func (c *ProcDeployConf) XMLExport(outPath string) error {
	worlds := make(map[int]*tcmWorldDeploy)
	zones := make(map[DeployKey]*tcmZoneDeploy)
