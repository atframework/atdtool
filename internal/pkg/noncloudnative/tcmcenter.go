...
// CenterConf tcm center configuration
type CenterConf struct {
	...
	ConfigSoName          string   `xml:"ConfigSoName,attr" json:"tcm_so_file"`
	...
	TcmDumpDir            string   `xml:"TcmDumpDir,attr" json:"tcm_dump_dir"`
...
func loadCenterConfig(filename string) (tcmConf, error) {
...
// validate verify that tcmcenter configuration data is illegal.
func (c *CenterConf) validate() error {
	return nil
}

// XMLExport export tcm center xml configuration
func (c *CenterConf) XMLExport(outPath string) error {
	outfile := path.Join(outPath, "tcmcenter.xml")
	output, err := xml.MarshalIndent(c, "", "    ")
	if err != nil {
		return fmt.Errorf("marshal(tcmcenter.xml): %v", err)
	}

	if err := os.WriteFile(outfile, output, 0644); err != nil {
		return fmt.Errorf("write(tcmcenter.xml): %v", err)
	}
